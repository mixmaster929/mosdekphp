...
$q = mysqli_query($con, "SELECT * FROM shop_brand $sqlWhere");
if(mysqli_num_rows($q) > 0) {
			...
			<table class="table table-hover table-sm" style="margin-top:30px; font-size:17px;">
				<thead class="thead" >
					<tr>
						...
						<th scope="col"><button onclick="massDel()" class="btn btn-sm btn-outline-danger py-0 m-1"> Mass Delete </button></th>
						<th><input type="checkbox" class="form-control-sm massdel" id="selectAll" onclick="selectAll()"></th>
					</tr>
					</thead>
					<tbody id="tyreBody">
					<?php echo $tr; ?>
			...
			<div class="form-group row" style = "margin-top:20	px">
                ...
                <div class="col-sm-8">
                    ...
                    if ($_SESSION['superAdmin'] == 1) {
                        echo '<input type="text" class="form-control modal_form" name="location" value="Moss Dekk AS">';
                    } else {
                        echo '<input type="text" class="form-control modal_form" name="location" value="Moss Dekk AS" readonly>';
                    }
                    ?>
                </div>
			</div>
...
function selectAll() {
	...
	var selectAll = selectAllEle[0].checked;

	var delIDs = $('.massdel');
	var delIDsArray = [];
	for (let index = 0; index < delIDs.length; index++) {
		var element = delIDs[index];
		element.checked = selectAll;
	}
}

function massDel(){
	var conf = confirm('Are you sure you want to delete selected entries?');
	if(conf == false) { return; }

	var delIDs = $('.massdel');
	var delIDsArray = [];
	for (let index = 0; index < delIDs.length; index++) {
		const element = delIDs[index];
		var ifChecked = element.checked;
		if(ifChecked){
			delIDsArray.push(element.id);
		}
	}
	if(delIDsArray.length < 1) {
		showAlert('danger', 'Entries not found');
		return;
	}
	
	showLoadingBar(1);
	var url = 'method=massDelete&id='+JSON.stringify(delIDsArray)+'&from=shop_brand';
	fetchA(url, function(result) {
		hideLoadingBar();
		var e = JSON.parse(result);
