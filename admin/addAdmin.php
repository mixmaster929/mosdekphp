...
function save(i, site) {
	...
	var superAdmin = $('#superAdmin'+type).val();
	//var email = $('#email'+type).val();
	if(username == '' || password == '' ) { alert('All fields are required'); return; }
	
	showLoadingBar(1);
	var url = 'method=saveAdmin&username='+username+'&password='+password+'&site='+site+'&location='+location+'&superAdmin='+superAdmin;
	fetchA(url, function(result) {
		hideLoadingBar();
...
function addAdmin(site, type) {
	...
	showLoadingBar(1);
	var url = 'method=addAdmin&username='+username+'&password='+password+'&site='+site+'&location='+location+'&superAdmin='+superAdmin;
	fetchA(url, function(result) {
		hideLoadingBar();
		var e = $.parseJSON(result);
