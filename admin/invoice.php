...
$year_tr = '';
$sqlWhere = ($_SESSION['superAdmin'] == 1) ? " " : "where locationID = 18 ";
$q_year = mysqli_query($con, "SELECT YEAR(changeDate) AS year FROM shop_invoice $sqlWhere GROUP BY YEAR(changeDate)");
...
$nopaidButton = '<button class="btn btn-sm btn-outline-danger py-0 m-0" >No Paid</button>';
    ...
    $sqlWhere = ($_SESSION['superAdmin'] == 1) ? " " : "locationID = 18 AND";
    if(!empty($yearID)){
        if(!empty($monthID)){
            $q = mysqli_query($con, "SELECT * FROM shop_invoice where $sqlWhere changeDate LIKE '%$yearID/$strMonth%' ORDER BY " .  $column . " " . $sort_order);
        }
        else{
            $q = mysqli_query($con, "SELECT * FROM shop_invoice where $sqlWhere changeDate = $yearID ORDER BY " .  $column . " " . $sort_order);
        }
    }
    else{
        $q = mysqli_query($con, "SELECT * FROM shop_invoice where $sqlWhere changeDate = '' ORDER BY " .  $column . " " . $sort_order);
