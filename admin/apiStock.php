...
$tr = '';
$i = 1;
$q = mysqli_query($con, "SELECT * FROM shop_stock_api");
if(mysqli_num_rows($q) > 0) {
	...
	foreach($fetch as $f) {
		...
		$ft = mysqli_fetch_array_n(mysqli_query($con, "SELECT * FROM shop_tyres_api WHERE id=$tyreID"), MYSQLI_ASSOC);
		$brand = $ft['brand'];
		$model = $ft['model'];
		$tyreSize = $ft['width'];
		...
		$tr .= '<tr id="tr'.$f['id'].'">';
		$tr .= '<td>'.$i.'</td>';
		$tr .= '<td style="max-width:200px;">'.$f['productdesc'].'</td>';
		...
		$tr .= '<td><span class="txtField'.$f['id'].'">'.$f['purchaseprice'].'</span>		
					<input type="number" min=0 class="form-control form-control-sm txtInput editField'.$f['id'].'" value="'.$f['purchaseprice'].'" id="purchasePrice'.$f['id'].'" style="display:none; font-weight:bold; max-width:100px;"></td>';
...
$q = mysqli_query($con, "SELECT * FROM shop_stock_api WHERE stock > 0"); 
if(mysqli_num_rows($q) > 0) {
	
	$fetch = mysqli_fetch_all_n($q, MYSQLI_ASSOC);
	foreach($fetch as $f) {
		$totalStock += $f['stock'];
			if(isset($f['purchaseprice'])) {
				$totalStockValue += $f['stock'] * (int)$f['purchaseprice']; //(int)$misc['purchasePrice'];
			}
	}
}
