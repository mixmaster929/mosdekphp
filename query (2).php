...
function registerCustomer() {
	...
	$q = mysqli_query($con, "INSERT INTO shop_customers (`id`, `createdOn`, `username`, `password`, `fullName`, `email`, `regNr`, `mobile`, `postCode`, `address`, `city`, `price`, `carType`, `misc`) 
												VALUES (NULL, '$today', '$username', '$password', '$fullName', '$email', '$regNr', '$mobile', '$postCode', '$address', '$city', '$price', '$carType', '')");
	if($q) {
		$r = ['success'];
		echo json_encode($r);
		return;
...
function tyreChangeDekkhotellOrderWithoutLogin() {
    ...
    $invoiceID = 0;
    if($paymentDone == 1 && $email != '' && $paymentMode != 'payAtShop') {
        //invoice
        $customerID = 0;
...
function tyreOrderWithoutLogin() {
	foreach($_POST as $key=>$value) {
		if($key == 'serviceIDs' || $key == 'serviceCounts' || $key == 'orgNr' || $key == 'orgNrDekk'  || $key == 'referenceDekk' || $key == 'addressLocation' || $key == 'postcodeLocation' || $key == 'cityLocation' || $key == 'reference') { continue; }
		if(p($value) == '') {
			$r = ['empty fields'];
			echo json_encode($r);
			return;
