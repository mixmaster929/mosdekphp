	...
	if($resArr->result == 'customer not found') {
		...
		return;
                ...
                if(is_object($resArr->arr) && $paymentMode != 'payAtShop'){
                    ...
                    $subject = $arr->subject;
                    $body = $arr->body;
                    $combinedString = $miscFailed . $toName . $subject;
                	$customError = 'mail send';
                    $errorFile = 'query.php';
                    $errorFunction = 'tyreChangeDekkhotellOrderWithoutLogin';
                    $qError = mysqli_query($con, "INSERT INTO `shop_errors` ( `id`,`regnr`,`workDate`,`paymentMode`,`function`,`file`,`customError`,`misc`,`customerinfo`,`created`) VALUES (NULL, '$regNr','$date $time','$paymentMode', '$errorFunction','$errorFile','$customError','$body','$combinedString', NULL)");
                	// Logg end
	...
	if(!is_object($resArr)) {
		...
		if($resArr->result == 'success' && $paymentDone == 1) {
			...
			$_SESSION['orderID'] = $orderID;
				...
				$acceptURL = 'https://dev.mossdekk.no/t1/?p=processOrder&email='.$email.'&totalTime=0&workType='.$workType.'&price='.$price.'&regNr='.$regNr.'&name='.$name.'&mobile='.$mobile.'&date='.$date.'&serviceIDs='.$serviceIDs.'&serviceCounts='.$serviceCounts.'&time2='.$time.'&tyres='.$tyres.'&locationID='.$locationID.'&postcodeLocation='.$postcodeLocation.'&addressLocation='.$addressLocation.'&cityLocation='.$cityLocation.'&orgNr='.$orgNr.'&reference='.$reference.'&paymentMode='.$paymentMode.'&selType='.$selType.'&tyreID='.$tyreID.'&tyreIDs='.$tyreIDs.'&privateCustomerID='.$privateCustomerID.'&offerID='.$offerID.'&type='.$type.'&serviceCounts='.$serviceCounts;
				$cancelURL = 'https://dev.mossdekk.no/t1/?p=processOrder&email='.$email.'&totalTime=0&workType='.$workType.'&price='.$price.'&regNr='.$regNr.'&name='.$name.'&mobile='.$mobile.'&date='.$date.'&serviceIDs='.$serviceIDs.'&serviceCounts='.$serviceCounts.'&time2='.$time.'&tyres='.$tyres.'&locationID='.$locationID.'&postcodeLocation='.$postcodeLocation.'&addressLocation='.$addressLocation.'&cityLocation='.$cityLocation.'&orgNr='.$orgNr.'&reference='.$reference.'&paymentMode='.$paymentMode.'&selType='.$selType.'&tyreID='.$tyreID.'&tyreIDs='.$tyreIDs.'&privateCustomerID='.$privateCustomerID.'&offerID='.$offerID.'&type='.$type.'&serviceCounts='.$serviceCounts; 
				$callbackURL = 'https://dev.mossdekk.no/t1/?pprocessOrderCallback&email='.$email.'&totalTime=0&workType='.$workType.'&price='.$price.'&regNr='.$regNr.'&name='.$name.'&mobile='.$mobile.'&date='.$date.'&serviceIDs='.$serviceIDs.'&serviceCounts='.$serviceCounts.'&time2='.$time.'&tyres='.$tyres.'&locationID='.$locationID.'&postcodeLocation='.$postcodeLocation.'&addressLocation='.$addressLocation.'&cityLocation='.$cityLocation.'&orgNr='.$orgNr.'&reference='.$reference.'&paymentMode='.$paymentMode.'&selType='.$selType.'&tyreID='.$tyreID.'&tyreIDs='.$tyreIDs.'&privateCustomerID='.$privateCustomerID.'&offerID='.$offerID.'&type='.$type.'&serviceCounts='.$serviceCounts;
